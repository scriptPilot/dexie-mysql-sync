...
export default async function sync(table, path, options = {}) {   
  ...
  table.hook('creating', (id, doc) => {
    ...
    doc.$created = doc.$created !== undefined ? doc.$created : Date.now()
    doc.$updated = doc.$updated !== undefined ? doc.$updated : Date.now()
    doc.$deleted = doc.$deleted !== undefined ? doc.$deleted : 0
    doc.$synchronized = doc.$synchronized !== undefined ? doc.$synchronized : 0
  ...
  table.hook('updating', (updates, id) => {
    const additionalUpdates = {
      ...
      $synchronized: updates.$synchronized !== undefined ? updates.$synchronized : 0
    }
